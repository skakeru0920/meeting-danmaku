...
1. 公式ドキュメントで以下を確認し、必須条件を満たす可能性がある SDK のうち、Mac で最も小さく試せるものを選ぶ(結論は `docs/adr/` に記録)
...
2. 第一候補として Web Meeting SDK の公式 sample を起動し、自分のミーティングへ join する。別の SDK は、Web で必須条件を満たせない場合にのみ試す
...
Express / WebSocket ライブラリ、overlay の構成、OBS 連携方法は後続 Spike で必要になってから決める。

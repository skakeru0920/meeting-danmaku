...
# 追記(2026-09-04): 表示方式の結論を更新

このメモの「もしかするとOBSすら不要？」で検討した Electron 案について、
当時は「MVPではOBSの方が安全そう」と結論していたが、**この判断を更新した**。

## 更新の理由

このメモを書いた時点では目的が「視聴者にコメントを見せる」に寄っていたが、
壁打ちを通じて目的が 2 つあることが明確になった。

1. **発表者自身がチャットを把握する**(画面共有中でチャット欄が見えないときも)
2. 視聴者にもコメントを見せる

目的 1 は OBS では満たせない。OBS は合成した映像を配信するもので、
発表者自身のデスクトップにコメントが出るわけではないため。
目的 1 を満たすにはデスクトップへ直接重ねる方式(Electron 透過ウィンドウ)が要る。

さらに、Electron のオーバーレイが**デスクトップ全体共有に映り込む**なら、
目的 2 も同じ仕組みで満たせて OBS が不要になる可能性がある。

## 現在の位置づけ

- Electron 透過オーバーレイ: **採用**(目的 1 のため必須)
- OBS Browser Source: **保留**。上記の映り込みが確認できれば不要、できなければ視聴者向けとして残す

このメモが挙げていた Electron 方式のデメリット
(macOS の画面キャプチャ仕様 / overlay window が共有に映るか / click-through /
Spaces / fullscreen / permissions)は、いずれも未解決の懸念として有効。
これらを `docs/spikes/002-electron-overlay-on-screen-share.md` で検証する。

なお、multi monitor はこのメモが挙げたとおり未検討のまま残っている(Spike 002 の範囲外)。

## Meeting SDK の Electron とは別の話

このメモの「Meeting SDK候補」に挙がっている Electron Meeting SDK と、
上記の Electron 透過オーバーレイは**別の判断**。前者はチャットを受信する手段、
後者は overlay を表示する手段。Spike 001 と Spike 002 で独立に決める。

...
- 状態: 完了(2026-09-06)
- 種別: 実装
- TODO: T-010(完了), T-011(完了), T-012(中止)
...
**結果**(2026-09-06): 完了。4 つすべて満たした。**MVP 完成。**

Zoom チャットへ Everyone 宛に投稿すると、発表者自身の画面にも、画面共有を
通して別 participant の画面にも弾幕が流れる。背景は透過していて DM は流れない。

#### Electron が検証用のダミー画面を表示したままだった

**通しで動かして初めて見つかった。** `src/electron/index.html` は T-004 で
透過だけを確かめるために作った独立ファイルで、`■ ここに弾幕が流れる ■` という
固定文字列が動いていただけだった。**`src/overlay/` を読み込んでいなかった**ので、
Zoom チャットも `/debug` の投稿も反映されない。

Plan 002 で「切り分けのため overlay とは切り離す」と決めたものが、そのまま
残っていた。T-004 の完了時に差し替えるべきだった。

Vite が配信する overlay を `loadURL` で読むように直した。`file://` で直接開くと
SSE の接続先(相対パスの `/events`)が解決できないため URL で読む。**ブラウザで
開くのと同じものが動く**ので、表示先による差異が出ない。そのぶん
**`npm run dev` を先に起動しておく必要がある。**

役目を終えた `src/electron/index.html` は削除した。`--frame` はこの検証用画面
専用の機能だったので併せて消した。スクリプト名も `electron:spike` から
`overlay` へ変えた。

#### マイクは最初から掴んでいなかった

T-011 の前に決着させることになっていた Icebox の件。`client.mute(true)` が
`6015 / 'no audio joined'` で失敗し、**ミュートする対象の音声接続が存在しない**
ことが分かった。`stopAudio()` が空振りしていたのも同じ理由。

**参加者リストのヘッドセット記号を「マイク ON」と読み違えていた。** あれは
音声未接続を表すもので、発表者と同じ Mac で動かしてもハウリングしない。
`mute` も `stopAudio` も呼ぶ必要がないので削除した。詳細は Plan 001 の訂正を読む。
...
**仮説は正しかった。** Plan 001 と Plan 002 で個別に検証した要素を繋ぐと、
目的 1 と目的 2 の両方を満たした(2026-09-06)。

体感遅延は気にならない範囲。**数値は測っていない。** 実運用で遅いと感じたら
測り直す。

### 分かったこと・後続への影響

- **通しで動かすまで気づかない種類の不具合があった。** Electron が T-004 の
  検証用ダミー画面を表示したままで、個別の要素はすべて動いていたのに繋がって
  いなかった。**検証用に切り離したものは、検証が終わった時点で差し替えるか
  消す。** 残しておくと本物のように見えてしまう
- **前提の思い込みは実測で崩れる。** マイクの件は「参加者リストのアイコンが
  ON に見えた」という観測から出発して、対処が要ると 1 か月近く Icebox に
  残っていた。実際にはマイクを掴んですらいなかった。**エラーコードまで
  読んで初めて分かった**(`6015 / no audio joined`)
- **Zoom タブを開いておく必要がある**のが現在の運用上の制約。閉じるとコメントが
  止まる。Electron の隠しウィンドウへ移す案が Icebox にある。使ってみて
  煩わしければ着手する

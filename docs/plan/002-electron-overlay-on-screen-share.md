...
**成功**(2026-09-06)。仮説は成立した。

**必須(1): 満たした。** `npm run electron:spike` で起動し、透過ウィンドウが
全アプリの手前に出ることと、クリックが背後へ透過することを確認した。
メニューバーと Dock の上にも弾幕が出る(画面全体を覆う設計なので想定どおり)。

**必須(2): 満たした。** Zoom のデスクトップ全体共有を通して弾幕が見えることを
確認した。**これで Electron 単体で目的 1(発表者が把握)と目的 2(視聴者に見せる)の
両方を満たせる。OBS は不要。**

**任意(3): 満たした。** Google Slides のフルスクリーン表示の上にも弾幕が出た。
Keynote そのものでは試していないが、フルスクリーンアプリの上に出るという
同じ性質の確認になっている。
...
- **視聴者への配信は Electron のデスクトップ共有だけで賄える。OBS は残さない。**
  → **T-012(OBS Browser Source)は中止**。TODO.md の注記どおり、
  T-004 が成功したので不要になった
- **フルスクリーン対応は MVP に含められる。** Google Slides の全画面表示の上に出た
- **overlay は Electron だけで使う。** OBS を使わないので、両方で使い回す必要はない。
  ただし overlay 側に表示先固有の処理を持ち込まない方針は変えない
  (`docs/decisions/` の「overlay の実装」)

赤枠と説明ラベルは検証用なので既定で消した。切り分けが要るときだけ
`npm run electron:spike -- --frame` で出す。

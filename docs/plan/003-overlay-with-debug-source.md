...
### T-008: Node.js → overlay へ配信

1. 既存の Express に配信エンドポイントを足す
2. `DebugCommentSource` の出力を接続中のクライアントへ broadcast する
3. T-005 の overlay のコメント供給部を受信に差し替える

**完了条件**: `npm start` → ブラウザで overlay を開くと `DebugCommentSource` の
コメントが流れる。再読み込みしても再接続して流れ続ける。

**結果**(2026-09-06): 完了。`GET /events` の SSE で配る。ブラウザで流れること、
`Cmd+R` で再読み込みしても再び流れ始めることを確認した。

#### ws から SSE へ変えた

**この Plan は当初「Express + ws」と書いていたが、SSE に変えた。**
着手前に選択肢を比べ直した結果で、Plan を書いた時点ではそこまで詰めていなかった。

| | ws | **SSE** | ポーリング |
|---|---|---|---|
| 依存追加 | 1 つ | **なし** | なし |
| 再接続 | 自前で実装 | **EventSource が標準で持つ** | 概念上不要 |
| 遅延 | 即時 | 即時 | 間隔ぶん遅れる |
| 双方向 | 可能 | 不可 | 不可 |
| サーバー側の状態 | 接続リスト | 接続リスト | **未送信キュー** |

SSE を選んだ理由:

- **サーバー → overlay の一方向で足りる。** overlay から送り返すものがない
- **依存が増えない。** `res.write()` と `EventSource` だけで書ける
- **完了条件の「再接続して流れ続ける」が標準機能で満たせる。**
  ws なら再接続ループとそのテストを書くことになる

ポーリングは遅延に加えて、サーバー側に「まだ渡していない分」を持つ必要があり、
チャット履歴を永続化しない方針と相性が悪いので採らなかった。

**双方向が要る条件**も検討した。(a) 送信が毎秒何十回の高頻度になる、
(b) 視聴者が操作して全員へ即反映する。どちらも現在の構想にない。
表示設定の変更は overlay 内で完結し、コメント削除や `/debug` からの投稿は
HTTP POST で足りる。必要になったら供給源クラスの差し替えで ws へ移れる。

#### 作ったもの

`src/server/broadcast.js` は**通信方式を知らない。** 送信関数を持つものなら
何でも扱えるので、将来 ws へ移ってもここは残る。

`isOverlayComment` で受信データの形を確かめてから描画へ渡す。壊れた 1 件で
弾幕全体が止まらないようにするため。**T-010 で Zoom の実データが
流れ始めるときに効く。**
...
**結果**(2026-09-06): 完了。`http://localhost:5173/src/debug/` から投稿すると
overlay に流れることを確認した。`<script>alert(1)</script>` は alert を出さず、
文字列としてそのまま弾幕になる。

#### 本文はサーバーでエスケープしない

**XSS の防御は overlay の `textContent` 一箇所に集約する。** 投稿経路の途中で
エスケープしない。サーバーで `&lt;script&gt;` へ変換すると、`textContent` が
それをそのまま文字として表示するので、画面に `&lt;script&gt;` が出てしまう
(二重エスケープ)。防御層を増やすほど安全になるわけではない例。

`src/server/comment.js` のテストで、`<script>` タグと `<img onerror>` が
**素通りする**ことを固定した。T-005 で描画側に入れた「要素にならない」テストと
対になっている。
...
| 項目 | 決め | 理由 |
|---|---|---|
| `text` が空・空白のみ・非文字列 | 400 | 流れても何も見えない |
| `text` の長さ | 500 文字まで。超えたら 400(切り詰めない) | 上限が無いと画面が埋まる |
| `id` | サーバーで採番(`post-<連番>`) | 同じ id を送られると DOM の対応付けが壊れる |
| `sender` | 未指定なら `debug`、50 文字で切り詰め | 表示が壊れない範囲に収める |
| `timestamp` | サーバーの受信時刻 | クライアント指定を信用しない |

壊れた JSON 用のエラーハンドラも入れた。Express の既定ハンドラは `SyntaxError` の
スタックトレースを HTML で返し、**絶対パスが漏れる**。

**dev-only なので認証は付けていない。** 誰でも投稿できるため開発機の外へ公開しない。

`DebugCommentSource` の自動投稿は止めていない。混ざるが、名前を入れれば
投稿分は見分けられる。止める引数は必要になってから足す。

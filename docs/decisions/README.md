...
参加者リストに 1 人増える(名前は `userName` で指定)。**カメラも音声も繋がない。**

**マイクは掴まないのでハウリングしない**(2026-09-06 に確定)。`join` しただけでは
音声接続が確立せず、`client.mute(true)` は `6015 / 'no audio joined'` で失敗する。
`client.stopAudio()` も同じ理由で空振りする。**どちらも呼ぶ必要がない。**

参加者リストに出るヘッドセット記号は「音声未接続」を表すもので、マイク ON では
ない。発表者と同じ Mac で動かして問題ない。

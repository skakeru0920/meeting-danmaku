...
実際に手戻りが起きた事例と、繰り返さないための対策。**追記のみ。**

### 公式ドキュメントより型定義を信じる

Plan 001 で、Web SDK のリファレンスページが JavaScript 描画のため取得できず、
npm パッケージの `.d.ts` を直接読んで判断した。結果としてこちらが正確だった。

- ドキュメントに書かれていない payload の形が型定義には書いてある
- ただし**型定義にも書かれていないこと**がある(`receiver` が Everyone のとき何を持つか)。
  そこは実機で観測するまで確定として書かない

### install 時の出力を読む

`@zoom/meetingsdk` は postinstall で「ボットや AI ノートテイカーはサポートしない」と
警告していたが、ドキュメントと型定義だけを見ていて最初は気づかなかった。
**依存を追加したら install の出力を確認する。**

### peerDependencies を確認してから依存を入れる

React 19 を入れて `ReactCurrentOwner` の実行時エラーで詰まった。
SDK は `react@18.2.0` を**範囲指定なしで**要求していた。

- 新しい SDK を入れたら、まず `peerDependencies` を読む
- 固定バージョン指定なら `^` を付けずにそのまま入れる

### エラーオブジェクトを潰さない

`error?.message ?? error` だけで表示していたら `[object Object]` になり、
原因が分からなかった。SDK のエラーは `{ type, reason, errorCode }` の形で来る。
**そのまま console に出す。** 実際、`reason: 'Meeting has not started'` と
`reason: 'on hold'` が原因特定の決め手になった。

### 環境要因を疑う前に、状態を疑う

Zoom の設定が変更できないという事象は、**会議中だったため**だった。
`stopAudio` が失敗したのは**待機室にいたため**だった。
権限やプラン制限を疑う前に、いまどういう状態かを確認する。

### 効率化できた点

- **確認項目を先に決めてから調べる。** 候補ごとに違う物差しで測らずに済み、
  比較表がそのまま結論になった([docs/research/](./docs/research/README.md) のルール)
- **却下理由を残す。** RTMS を「realtime media だから音声用」と早合点して
  一度は候補から外したが、後に再調査してチャット対応と判明した。
  却下理由が書いてあったので何を見落としたかがすぐ分かった
- **未確認事項を明示して先へ進む。** 全部を確定させてから実装するのではなく、
  「これは実機で確かめる」と書いて進めた。実装が先に進み、
  観測した実データがそのまま次のタスクの材料になった
